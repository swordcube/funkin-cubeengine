...
//this is unused cuz i am dumb
//also not finished lmao
...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
        ...
        for (i in 0...GamePrefs.noteShit.length) {
            var yPos:Float = (165 * i) + 35;
            for (j in 0...3) {
                var optionText:Alphabet = new Alphabet(0, yPos, Std.string(GamePrefs.noteShit[i][0][j]));
                optionText.x = posX + (225 * j) + 100 - ((optionText.lettersArray.length * 90) / 2);
                grpNumbers.add(optionText);
            }
			var note:FlxSprite = new FlxSprite(yPos, posX - 70);
			skin = GamePrefs.noteShit[i][1];
			...
			newShader.hue = GamePrefs.noteShit[i][1][0] / 360;
			newShader.saturation = GamePrefs.noteShit[i][1][1] / 100;
			newShader.brightness = GamePrefs.noteShit[i][1][2] / 100;
	...
	function changeSelection(change:Int = 0) {
		...
		curValue = GamePrefs.noteShit[curSelected][0][typeSelected];
		updateValue();
	...
	function resetValue(selected:Int, type:Int) {
		curValue = 0;
		GamePrefs.noteShit[selected][0][type] = 0;
		switch(type) {
	...
	function updateValue(change:Float = 0) {
		...
		GamePrefs.noteShit[curSelected][0][typeSelected] = roundedValue;
