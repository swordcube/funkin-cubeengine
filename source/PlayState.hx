...
class PlayState extends MusicBeatState
{
	...
	public static var usedPracticeMode:Bool = false;
	public static var deathCounter:Int = 0;
	public static var stupidBotplayArrowTimerShit:Int = 0;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage':
			{
				...
				stageFront.antialiasing = GamePrefs.antialiasing;
				...
				stageCurtains.antialiasing = GamePrefs.antialiasing;
			...
			case 'spooky': 
			{
				  ...
				  halloweenBG.antialiasing = GamePrefs.antialiasing;
				  add(halloweenBG);
			}
			...
			case 'philly': 
			{
				  ...
				  city.updateHitbox();
				  city.antialiasing = GamePrefs.antialiasing;
				  add(city);
				  ...
				  for (i in 0...5)
				  {
					  ...
					  light.updateHitbox();
					  light.antialiasing = GamePrefs.antialiasing;
					  phillyCityLights.add(light);
				  }

				  var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('philly/behindTrain'));
				  streetBehind.antialiasing = GamePrefs.antialiasing;
				  add(streetBehind);

				  phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('philly/train'));
				  phillyTrain.antialiasing = GamePrefs.antialiasing;
				  add(phillyTrain);
				  ...
				  street.antialiasing = GamePrefs.antialiasing;
				  add(street);
			  }
			  ...
			  case 'limo':
			  {
				  ...
				  skyBG.antialiasing = GamePrefs.antialiasing;
				  add(skyBG);
				  ...
				  bgLimo.antialiasing = GamePrefs.antialiasing;
				  add(bgLimo);
				  ...
				  overlayShit.antialiasing = GamePrefs.antialiasing;
				  // add(overlayShit);
				  ...
				  limo.antialiasing = GamePrefs.antialiasing;

				  fastCar = new FlxSprite(-300, 160).loadGraphic(Paths.image('limo/fastCarLol'));
				  fastCar.antialiasing = GamePrefs.antialiasing;
				  limo.antialiasing = GamePrefs.antialiasing;
				  //add(limo);
			  }
			  ...
			  case 'mall':
			  {
				  ...
				  upperBoppers.antialiasing = GamePrefs.antialiasing;
				  ...
				  bgEscalator.antialiasing = GamePrefs.antialiasing;
				  ...
				  tree.antialiasing = GamePrefs.antialiasing;
				  ...
				  bottomBoppers.antialiasing = GamePrefs.antialiasing;
				  ...
				  fgSnow.active = false;
				  fgSnow.antialiasing = GamePrefs.antialiasing;
				  add(fgSnow);

				  santa = new FlxSprite(-840, 150);
				  santa.frames = Paths.getSparrowAtlas('christmas/santa');
				  santa.animation.addByPrefix('idle', 'santa idle in fear', 24, false);
				  santa.antialiasing = GamePrefs.antialiasing;
				  add(santa);
			  }
			  ...
			  case 'mallEvil':
			  {
				  ...
				  evilTree.antialiasing = GamePrefs.antialiasing;
				  evilTree.scrollFactor.set(0.2, 0.2);
				  add(evilTree);

				  var evilSnow:FlxSprite = new FlxSprite(-200, 700).loadGraphic(Paths.image("christmas/evilSnow"));
				  evilSnow.antialiasing = GamePrefs.antialiasing;
				  add(evilSnow);
			  }
		...
		switch (curStage)
		{
			case 'limo':
				...
				if(!GamePrefs.optimization) resetFastCar();
				if(!GamePrefs.optimization) add(fastCar);
		...
		if(!GamePrefs.optimization) add(gf);

		// Shitty layering but whatev it works LOL
		if (curStage == 'limo' && !GamePrefs.optimization)
			add(limo);

		if(!GamePrefs.optimization) add(dad);
		if(!GamePrefs.optimization) add(boyfriend);
		...
		// THE HEALTH BAR HAS COLOR :euphoria:
		// healthBar
		add(healthBar);
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					babyArrow.antialiasing = GamePrefs.antialiasing;
	...
	public var rating2:String = '?';
	public var accuracy:Float = 0;
	
	public function CalculateAccuracy() {
		accuracy = songScore / ((songHits + songMisses - ghostMisses) * 350);
		...
		if(Math.isNaN(accuracy)) {
			...
			accuracy = 1;
			rating = ratingStuff[ratingStuff.length-1][0]; //Uses last string
		} else {
			for (i in 0...ratingStuff.length-1) {
				if(accuracy < ratingStuff[i][1]) {
					rating = ratingStuff[i][0];
					break;
				}
			}
		}
		
		rating2 = ratingStuff2[0]; // setting this to smth just in case the script below does nothing
		if (songMisses == 0 && goods == 0 && bads == 0 && shits == 0) {
		
			rating2 = ratingStuff2[4];
			
		} else if (songMisses == 0 && goods >= 1 && bads == 0 && shits == 0) {
		
			rating2 = ratingStuff2[3];
			
		} else if (songMisses == 0) {
		
			rating2 = ratingStuff2[2];
			
		} else if (songMisses < 10) {
		
			rating2 = ratingStuff2[1];
			
		} else {
		
			rating2 = ratingStuff2[0];
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case 'philly':
				if (trainMoving && !GamePrefs.optimization)
				{
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.isSustainNote
					&& (GamePrefs.downscroll && daNote.y > strumLine.y || !GamePrefs.downscroll && daNote.y < strumLine.y)
				...
				if(GamePrefs.downscroll) doKill = daNote.y > FlxG.height;
	...
	function endSong():Void
	{
		...
		//{
			...
			{
				#if !switch
				if(songMultiplier >= 1 && !usedBotplay && !usedPracticeMode) {
	...
	private function popUpScore(strumtime:Float, note:Note):Void
	{
		...
		if(!botplay) songScore += score;
		songHits += 1;
		if(!botplay) CalculateAccuracy();
	...
	{
		if (trainSound.time >= 4700 && !GamePrefs.optimization)
		{
			startedMoving = true;
			gf.playAnim('hairBlow');
		}

		if (startedMoving && !GamePrefs.optimization)
		{
			phillyTrain.x -= 400;
	...
	override function beatHit()
	{
		...
		if (generatedMusic)
		{
			notes.sort(FlxSort.byY, GamePrefs.downscroll ? FlxSort.ASCENDING : FlxSort.DESCENDING);
		}
		...
		{
			case 'school':
				if(!GamePrefs.optimization) bgGirls.dance();
			...
			case 'limo':
				...
				if (FlxG.random.bool(10) && !GamePrefs.optimization && fastCarCanDrive)
					fastCarDrive();
			case "philly":
				if (!trainMoving && !GamePrefs.optimization)
					trainCooldown += 1;

				if (curBeat % 4 == 0 && !GamePrefs.optimization)
				{
				...
				if (curBeat % 8 == 4 && !GamePrefs.optimization && FlxG.random.bool(30) && !trainMoving && trainCooldown > 8)
				{
